...
class SessionWorker:
    """
    Dedicated worker thread for a session that ensures all operations
    happen in the same thread (required for Playwright/greenlet compatibility).
    """
    ...
    def start(self) -> "Environment":
        """Start the worker thread and create the environment."""
        self._running = True
        self._thread = threading.Thread(target=self._worker_loop, daemon=True)
        self._thread.start()

        # Create environment in the worker thread
        result = self.submit(self._create_env)
    ...
    def _create_env(self) -> "Environment":
        """Create environment with event loop (called in worker thread)."""
        try:
    ...
    def _worker_loop(self) -> None:
        """Main worker loop that processes tasks in the dedicated thread."""
        # Set up event loop for this thread (for Playwright)
        try:
            asyncio.get_running_loop()
        except RuntimeError:
            loop = asyncio.new_event_loop()
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            pass  # Queue may be broken if thread died
        if self._thread and self._thread.is_alive():
            self._thread.join(timeout=2.0)
            # Force cleanup if thread doesn't stop gracefully
            if self._thread.is_alive():
                print("[WORKER DEBUG] Worker thread did not stop gracefully")
...
class HTTPEnvServer:
    ...
    async def _cleanup_stale_sessions(self) -> None:
        """
        Clean up any stale sessions where the worker thread has died.
        This helps recover from clients that disconnected abruptly.
        """
        stale_session_ids = []
        for session_id, worker in self._session_workers.items():
            if not worker.is_alive:
                stale_session_ids.append(session_id)
                print(f"[WS DEBUG] Found stale session: {session_id}")

        for session_id in stale_session_ids:
            print(f"[WS DEBUG] Cleaning up stale session: {session_id}")
            # Remove from tracking dicts directly (we're already holding the lock)
            if session_id in self._session_workers:
                worker = self._session_workers.pop(session_id)
                worker.stop()
    ...
    async def _create_session(self) -> tuple[str, Environment]:
        ...
        Returns:
            Tuple of (session_id, environment, worker)
        ...
        async with self._session_lock:
            # First, clean up any stale sessions from abrupt disconnects
            ...
            # Create a dedicated worker thread for this session
            # This ensures all Playwright operations happen in the same thread
            worker = SessionWorker(self._env_factory)

            try:
                # Start worker and create environment (blocking but runs in worker thread)
                loop = asyncio.get_event_loop()
                env = await loop.run_in_executor(None, worker.start)
            except Exception as e:
                worker.stop()
    ...
    async def _destroy_session(self, session_id: str) -> None:
        ...
        """
        print(f"[WS DEBUG] Destroying session: {session_id}")
        async with self._session_lock:
            # Close environment in its worker thread to avoid greenlet issues
            if session_id in self._session_workers:
                worker = self._session_workers.pop(session_id)
                if session_id in self._sessions:
                    env = self._sessions.pop(session_id)
                    try:
                        # Try to close env in worker thread, but with a timeout
                        if worker.is_alive:
                            worker.submit(env.close)
                        else:
                            # Worker is dead, try direct close
                            try:
                                env.close()
                            except Exception:
                                pass
                    except Exception as e:
                        print(f"[WS DEBUG] Error closing env: {e}")
                # Always stop the worker
                worker.stop()
                print(f"[WS DEBUG] Worker stopped for session: {session_id}")
            elif session_id in self._sessions:
                # Fallback if no worker (shouldn't happen)
                env = self._sessions.pop(session_id)
                try:
                    env.close()
                except Exception:
            ...
            self._session_info.pop(session_id, None)
            print(f"[WS DEBUG] Session destroyed: {session_id}")
    ...
    async def _run_in_session_worker(
        self, session_id: str, func: Callable[..., Observation], *args, **kwargs
    ) -> Observation:
        """Run a synchronous function in the session's dedicated worker thread.

        This ensures all operations for a session happen in the same thread,
        which is required for Playwright/greenlet compatibility.
        """
        worker = self._session_workers.get(session_id)
        if worker is None:
            raise RuntimeError(f"No worker found for session {session_id}")

        def run_in_worker():
            print(f"[WORKER DEBUG] Submitting {func.__name__} to worker...")
            try:
                result = worker.submit(func, *args, **kwargs)
                print(f"[WORKER DEBUG] {func.__name__} completed successfully")
                return result
            except Exception as e:
                print(f"[WORKER DEBUG] {func.__name__} failed with: {e}")
                raise

        loop = asyncio.get_event_loop()
        # Run the worker.submit in a thread pool to avoid blocking the event loop
        return await loop.run_in_executor(None, run_in_worker)
...
all schema information needed to interact with the environment.
        ...
        async def websocket_endpoint(websocket: WebSocket):
            ...
            try:
                ...
                print("[WS DEBUG] Creating session...")
                session_id, session_env = await self._create_session()
                print(f"[WS DEBUG] Session created: {session_id}")

                while True:
                    # Receive message from client
                    print("[WS DEBUG] Waiting for message...")
                    raw_message = await websocket.receive_text()
                    print(f"[WS DEBUG] Received message type: {raw_message[:50] if raw_message else 'empty'}...")
                    ...
                    try:
                        match msg_type:
                            case "reset":
                                print("[WS DEBUG] Processing reset message...")
                                ...
                                if is_async:
                                    print("[WS DEBUG] Using async reset...")
                                    ...
                                    print("[WS DEBUG] Using sync reset in worker...")
                                    sig = inspect.signature(session_env.reset)
                                    valid_kwargs = self._get_valid_kwargs(sig, msg.data)
                                    observation = await self._run_in_session_worker(
                                        session_id, session_env.reset, **valid_kwargs
                                    )
                                print("[WS DEBUG] Reset completed, got observation")

                                self._update_session_activity(session_id)

                                print("[WS DEBUG] Serializing observation...")
                                response = WSObservationResponse(
                                    data=serialize_observation(observation)
                                )
                                print(f"[WS DEBUG] Response prepared, size: {len(response.model_dump_json())} bytes")
                        ...
                        print("[WS DEBUG] Sending response...")
                        await websocket.send_text(response.model_dump_json())
                        print("[WS DEBUG] Response sent successfully")
